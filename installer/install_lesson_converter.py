...
APP_NAME = "DRYDefense"
DISPLAY_NAME = "DRY Defense"
APP_VERSION = "1.0.0"
DESKTOP_SHORTCUT = Path.home() / "Desktop" / f"{DISPLAY_NAME}.lnk"
INSTALL_DIR = Path.home() / "AppData" / "Local" / "DRYDefense"
BATCH_LAUNCHER = INSTALL_DIR / "launch_dry_defense.bat"
ICON_PATH = ROOT / "assets" / "dry_defense_icon.ico"
...
def install() -> None:
    ...
    if ICON_PATH.exists():
        shutil.copy2(ICON_PATH, INSTALL_DIR / "dry_defense_icon.ico")
...
$Shortcut.IconLocation = '{INSTALL_DIR / 'dry_defense_icon.ico'}'
$Shortcut.Description = '{DISPLAY_NAME} {APP_VERSION}'
$Shortcut.Save()
"""
    subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", powershell_script], check=False)

    print(f"Installed {DISPLAY_NAME} v{APP_VERSION} to {INSTALL_DIR}")
